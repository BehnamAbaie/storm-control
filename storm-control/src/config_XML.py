...
import settings_function


#################################
######## main settings ##########
#################################
listCh = [['DAPI','0.1',False],['A488','1.0',False],['Cy3','1.0',False],['Cy5','1.0',True],['A750','1.0',False]]
# Nz = 10
# zstep = 1.0
Nz = 81             # number of z steps
zstep = 0.150      # delta z in units of micron
em_filter_wheel_boolean = True    # do we need an emission filter wheel?


sample_holder = 'uni_holder'  # sample holder; options are uni_holder or bioptech
oversampling = "100"
valve_loop = False
...
module_name.text = 'storm_control.sc_hardware.nationalInstruments.nidaqModule3'
...
camera_fire_pin1 = ET.SubElement(timing,'camera_fire_pin1',type='string')
camera_fire_pin1.text = '/Dev2/PFI8'
counter1 = ET.SubElement(timing,'counter1',type='string')
counter1.text = '/Dev2/ctr0'
counter_out1 = ET.SubElement(timing,'counter_out1',type='string')
counter_out1.text = '/Dev2/PFI12'
...
for ii in range(3,5):
    illumination = ET.SubElement(configuration,listI[ii][0])
    ao_task1 = ET.SubElement(illumination,'ao_task1')
    source = ET.SubElement(ao_task1,'source',type='string')
    source.text = listI[ii][1]
    do_task1 = ET.SubElement(illumination,'do_task1')
    source = ET.SubElement(do_task1,'source',type='string')
    source.text = listI[ii][2]
...
#################################
if em_filter_wheel_boolean:
    filter_wheel = ET.SubElement(modules,'filter_wheel')      
    module_name = ET.SubElement(filter_wheel,'module_name',type='string')
    module_name.text = 'storm_control.hal4000.miscControl.filterWheel'
    class_name = ET.SubElement(filter_wheel,'class_name',type='string')
    class_name.text = 'FilterWheel'  
    configuration = ET.SubElement(filter_wheel,'configuration')
    filter_wheel_fn = ET.SubElement(configuration,'filter_wheel_fn',type='string')
    filter_wheel_fn.text = 'LudlFW'
    filters = ET.SubElement(configuration,'filters',type='string')
    filters.text = 'DAPI,A488,Cy3,Cy5,A750,None'   
...
#################################
if em_filter_wheel_boolean:
    LudlFW = ET.SubElement(modules,'LudlFW')      
    module_name = ET.SubElement(LudlFW,'module_name',type='string')
    module_name.text = 'storm_control.sc_hardware.ludl.ludlEMFilterWheelModule'
    class_name = ET.SubElement(LudlFW,'class_name',type='string')
    class_name.text = 'LudlEMFilterWheelModule'  
    configuration = ET.SubElement(LudlFW,'configuration')
    baudrate  = ET.SubElement(configuration,'baudrate',type='int')
    baudrate.text = '9600'
    maximum  = ET.SubElement(configuration,'maximum',type='int')
    maximum.text = '6'
    port  = ET.SubElement(configuration,'port',type='string')
    port.text = 'COM17' 
...
listCh_config = [['400','0,0,139','daq.ilm400.ao_task',str(float(listCh[0][1])*4.99),'0.0','daq.ilm400.do_task'],
                 ['470','0,0,255','daq.ilm470.ao_task',str(float(listCh[1][1])*4.99),'0.0','daq.ilm470.do_task'],
                 ['545','127,255,0','daq.ilm545.ao_task',str(float(listCh[2][1])*4.99),'0.0','daq.ilm545.do_task'],
                 ['630','255,48,48','daq.ilm630.ao_task1',str(float(listCh[3][1])*4.99),'0.0','daq.ilm630.do_task1'],
                 ['730','139,26,26','daq.ilm730.ao_task1',str(float(listCh[4][1])*4.99),'0.0','daq.ilm730.do_task1']]
ch_counter = 0
for ii in range(len(listCh)):
    if listCh[ii][2]:
        ch = ET.SubElement(configuration,'ch'+str(ch_counter+1))
        ...
        amplitude_modulation_daq = ET.SubElement(ch,'amplitude_modulation_daq')
        hw_fn_name = ET.SubElement(amplitude_modulation_daq,'hw_fn_name',type='string')
        hw_fn_name.text = listCh_config[ii][2]
        max_voltage = ET.SubElement(amplitude_modulation_daq,'max_voltage',type='float')
        max_voltage.text = listCh_config[ii][3]
        min_voltage = ET.SubElement(amplitude_modulation_daq,'min_voltage',type='float')
...
###### Marzhauser Stage #########
#################################
marzhauser_stage = ET.SubElement(modules,'marzhauser_stage')      
module_name = ET.SubElement(marzhauser_stage,'module_name',type='string')
module_name.text = 'storm_control.sc_hardware.marzhauser.marzhauserModule'
class_name = ET.SubElement(marzhauser_stage,'class_name',type='string')
class_name.text = 'MarzhauserStage'  
configuration = ET.SubElement(marzhauser_stage,'configuration')
baudrate = ET.SubElement(configuration,'baudrate',type='int')
baudrate.text = '57600'
port = ET.SubElement(configuration,'port',type='string')
port.text = 'COM18'
velocity = ET.SubElement(configuration,'velocity',type='float')
velocity.text = '10.0'
x_limit = ET.SubElement(configuration,'x_limit',type='float')
x_limit.text = '10001.'
y_limit = ET.SubElement(configuration,'y_limit',type='float')
y_limit.text = '10001.'
joystickOnOff = ET.SubElement(configuration,'joystickOnOff',type='boolean')
joystickOnOff.text = 'False'
...
stage_functionality.text = 'marzhauser_stage'
...
for ii in range(Nz):
    ...
    if listCh[0][2]:
        ...
        ET.SubElement(event, "channel").text = str(shutter_counter)
        ET.SubElement(event, "power").text = "1.0"
        ET.SubElement(event, "on").text = str(float(shutter_counter+ii*Nch))
        ET.SubElement(event, "off").text = str(float(shutter_counter+0.1+ii*Nch))
    ...
    if listCh[1][2]:
        ...
        ET.SubElement(event, "power").text = "1.0"
        ET.SubElement(event, "on").text = str(float(shutter_counter+ii*Nch))
        ET.SubElement(event, "off").text = str(float(shutter_counter+0.95+ii*Nch))
...
if em_filter_wheel_boolean:
    for ii in range(Nch):
        if listCh_final[ii][2]:    
            item = ET.SubElement(loop,'item',name='Movie' + str(ii))
else:
    item = ET.SubElement(loop,'item',name='Movie0')
    

if em_filter_wheel_boolean:
    for ii in range(len(listCh_final)):
        if listCh_final[ii][2]:
            item = ET.SubElement(root,'item',name='Movie' + str(ii))
            movie = ET.SubElement(item,'movie')
            name = ET.SubElement(movie,'name',increment='Yes')
            name.text = listCh_final[ii][0]
            length = ET.SubElement(movie,'length')
            length.text = str(Nz)
            parameters = ET.SubElement(movie,'parameters')
            parameters.text = 'setting_' + str(ii)
            variable_entry = ET.SubElement(movie,'variable_entry',name='Movie Loop')
else:
    item = ET.SubElement(root,'item',name='Movie0')
    movie = ET.SubElement(item,'movie')
    name = ET.SubElement(movie,'name',increment='Yes')
    name.text = 'Tile'
    length = ET.SubElement(movie,'length')
    length.text = str(Nch*Nz)
    variable_entry = ET.SubElement(movie,'variable_entry',name='Movie Loop')
...
#################################
if em_filter_wheel_boolean:
    settings_function.settings(listCh,listCh_final,Nch,Nz,zstep,oversampling)
