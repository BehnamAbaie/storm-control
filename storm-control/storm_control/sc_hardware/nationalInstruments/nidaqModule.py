...
Modified by Behnam Abaie on 02/24/2021
Lines modified: 378-405 and 474-502
"""
import numpy
            ...
            def startAoTask():
                ...
                try:
                    ...
                    for ii in range(len(source_list_unified)):
                        if ii == 0:
                            analog_data_temp = analog_data[:source_list_dict[source_list_unified[0]]]
                        else:
                            analog_data_temp = analog_data[source_list_dict[source_list_unified[0]]:]
                    
                        self.ao_task = []
                        self.ao_task = nicontrol.AnalogWaveformOutput(source = analog_data_temp[0].getSource())
                        for i in range(1, len(analog_data_temp)):
                            self.ao_task.addChannel(source = analog_data_temp[i].getSource())
        ...
        if (len(self.digital_waveforms) > 0):
            ...
            def startDoTask():
                ...
                try:
                    ...
                    for ii in range(len(source_list_unified)):
                        if ii == 0:
                            digital_data_temp = digital_data[:source_list_dict[source_list_unified[0]]]
                        else:
                            digital_data_temp = digital_data[source_list_dict[source_list_unified[0]]:]
                    
                        # Create channels.
                        self.do_task = nicontrol.DigitalWaveformOutput(source = digital_data_temp[0].getSource())
                        for i in range(len(digital_data_temp) - 1):
                            self.do_task.addChannel(source = digital_data_temp[i+1].getSource())
