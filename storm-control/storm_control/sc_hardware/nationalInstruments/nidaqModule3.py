...
class AOTaskFunctionality(NidaqFunctionality,amplitudeModule.AmplitudeFunctionality):
    """
...
class AOTaskFunctionality1(NidaqFunctionality,amplitudeModule.AmplitudeFunctionality):
    """
    Asynchronous output of a voltage on a single line.
    """
    def createTask(self):
...
class NidaqModule(daqModule.DaqModule,amplitudeModule.AmplitudeModule):
    ...
    def __init__(self, module_params = None, qt_settings = None, **kwds):
        ...
        for fn_name in configuration.getAttrs():
            ...
            for task_name in task.getAttrs():
                ...
                elif (task_name == "ao_task1"):
                    ni_task = AOTaskFunctionality1(source = task_params.get("source"))
    ...
    def filmTiming(self, message):
        ...
        if self.run_shutters:
            ...
            if (self.oversampling == 1):
                wv_clock1 = self.timing.get("camera_fire_pin1")
            else:
                wv_clock1 = self.timing.get("counter_out1")

            daq1_ang = False
            daq2_ang = False
            for data in self.analog_waveforms:
                if data.getSource().rsplit('/')[1] == "Dev1":
                    daq1_ang = True        
            ...
            daq1_dig = False
            daq2_dig = False
            for data in self.digital_waveforms:
                if data.getSource().rsplit('/')[1] == "Dev1":
                    daq1_dig = True        
                if data.getSource().rsplit('/')[1] == "Dev2":
                    daq2_dig = True 
            ...
            # Setup the counter1.
            if daq2_ang:
                self.setupCounter1(frequency)
            
            # Setup analog waveforms.
            if daq1_ang:
                self.setupAnalog(frequency, wv_clock)
            
            # Setup analog waveforms1.
            if daq2_ang:
                self.setupAnalog1(frequency, wv_clock1)

            # Setup digital waveforms.
            if daq1_dig:
                self.setupDigital(frequency, wv_clock)
            
            # Setup digital waveforms1.
            if daq2_dig:
                self.setupDigital1(frequency, wv_clock1)
    ...
    def setupAnalog(self, frequency, wv_clock):
        ...
        if (len(self.analog_waveforms) > 0):
            ...
            for data in analog_data:
                if data.getSource().rsplit('/')[1] == "Dev1":
                    analog_data_temp.append(data)
            analog_data = analog_data_temp
    ...
    def setupAnalog1(self, frequency, wv_clock):
        """
        Configures for analog waveform output.
        """
        self.ao_task1 = None
        if (len(self.analog_waveforms) > 0):
            ...
            def startAoTask():
                ...
                try:
                    # Create channels.
                    self.ao_task1 = nicontrol.AnalogWaveformOutput(source = analog_data[0].getSource())
                    for i in range(1, len(analog_data)):
                        self.ao_task1.addChannel(source = analog_data[i].getSource())

                    # Add waveforms
                    self.ao_task1.setWaveforms(waveforms = waveforms,
                                              sample_rate = frequency,
                                              clock = wv_clock)

                    # Start task.
                    self.ao_task1.startTask()
                except nicontrol.NIException as exception:
                    print(exception)
                    return True
    ...
    def setupCounter1(self, frequency):
        """
        Configures the counter for filming.
        """
        self.ct_task1 = None
        if (self.oversampling > 1):
            def startCtTask():
                try:
                    self.ct_task1 = nicontrol.CounterOutput(source = self.timing.get("counter1"), 
                                                           frequency = frequency, 
                                                           duty_cycle = 0.5)
                    self.ct_task1.setCounter(number_samples = self.oversampling)

                    rising_edge1 = True
                    if self.timing.has("rising_edge1"):
                        rising_edge1 = self.timing.get("rising_edge1")

                    self.ct_task1.setTrigger(
                            trigger_source = self.timing.get("camera_fire_pin1"),
                            rising_edge = rising_edge1)
                    self.ct_task1.startTask()
                except nicontrol.NIException as exception:
                    print(exception)
                    return True
    ...
    def setupDigital1(self, frequency, wv_clock):
        ...
        if (len(self.digital_waveforms) > 0):
            ...
            def startDoTask():
                ...
                try:
                    # Create channels.
                    self.do_task1 = nicontrol.DigitalWaveformOutput(source = digital_data[0].getSource())
                    for i in range(len(digital_data) - 1):
                        self.do_task1.addChannel(source = digital_data[i+1].getSource())
    ...
    def stopFilm(self, message):
        ...
        """
        if self.run_shutters:
            for task in [self.ct_task, self.ct_task1, self.ao_task, self.ao_task1, self.do_task, self.do_task1]:
                if task is not None:
                    try:
                        task.stopTask()
                        task.clearTask()
