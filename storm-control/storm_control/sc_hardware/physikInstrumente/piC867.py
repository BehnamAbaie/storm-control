...
import sys
sys.path.append(r'C:\Users\MERFISH\Documents\GitHub\PI\PIPython-1.5.0.10')
...
class piC867(object):
    ...
    def goAbsolute(self, x, y):
        if self.good:
            ...
            if X >= self.rangemin_x['1'] and X <= self.rangemax_x['1'] and Y >= self.rangemin_y['1'] and Y <= self.rangemax_y['1']:
                self.pidevice2.MOV(1, X)
                self.pidevice1.MOV(1, Y)
            else:
    ...
    def goRelative(self, dx, dy):
        if self.good:
            # self.jog(0.0,0.0)
            x0 = self.pidevice2.qPOS(1)[1]  # query single axis [need to check units. Also, shouldn't this be zero indexed?]
            y0 = self.pidevice1.qPOS(1)[1]  # query single axis
    ...
    def position(self):
        if self.good:
            x0 = self.pidevice2.qPOS(1)[1] * self.unit_to_um  # query single axis and convert to um
            y0 = self.pidevice1.qPOS(1)[1] * self.unit_to_um  # query single axis and convert to um
            return {"x" : x0,
                "y" : y0}
    ## jog
    #
